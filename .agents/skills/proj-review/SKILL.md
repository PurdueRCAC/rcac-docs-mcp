...
name: proj-review
description: >-
  Adversarial QA of a completed rcac-docs-mcp feature branch. Delegates the correctness pass to a
  FRESH reviewer subagent that sees only GOAL.md, the branch diff, the AGENTS.md invariant checklist
  and the runnable repo — NOT PLAN.md/TECH.md, which avoids grading-its-own-homework. The reviewer
  refutes each finding and cites executed commands run in a sandboxed site; CONFIRMED findings loop
  back to /proj-build; a high-blast-radius file or a tool-surface, unauthenticated-posture or
  index-integrity invariant forces a human gate. Fourth step of the software factory (see
  .agents/factory/review-rubric.md).
disable-model-invocation: true
argument-hint: "[debate] [completeness] [status]"
allowed-tools: Read, Grep, Glob, Write, Agent, ReportFindings, AskUserQuestion, Bash(git status *), Bash(git branch *), Bash(git log *), Bash(git diff *), Bash(git rev-parse *), Bash(git submodule *), Bash(git add *), Bash(git commit *), Bash(uv run *), Bash(.agents/factory/bin/*), Bash(echo *), Bash(grep *), Bash(test *)
---

# proj-review — adversarial QA (clean context)

## When to Use

Invoke `/proj-review` when a branch's `TECH.md` is fully built (`status: in_review`). **Best run in a
...
This project has a real test suite — 107 tests — and that lowers the bar for this pass less than it
looks. The suite proves the server behaves; it does not check that the five places stating the tool
contract still agree with each other, it does not notice prose rotting in `INSTRUCTIONS.md`, and it
exits 0 with 31 of its tests skipped when the RCAC-Docs submodule is absent. The reviewer reports the
passed and skipped counts, not the exit status.
...
perform the correctness pass yourself in a clean context** (you lose delegated blindness — compensate
with executed evidence, per the rubric); skip `ReportFindings` (`REVIEW.md` is the durable record).
...
- Files changed vs main (spec-excluded): !`git diff --name-only main...HEAD -- . ':(exclude)spec/' 2>/dev/null | grep . || echo "(none, or main is not the base -- confirm in TECH.md)"`
- RCAC-Docs fixture: !`test -d tests/fixtures/RCAC-Docs/docs && echo "initialized" || echo "ABSENT -- 31 tests will skip and temp_site.sh will exit 3; run: git submodule update --init tests/fixtures/RCAC-Docs"`
...
- **External verification is the spine.** Every finding must cite an executed command. No
  assertion-only findings. `uv run pytest -q` alone is not one: it exits 0 with the submodule absent,
  so the counts are part of the evidence.
- **Every drive is sandboxed.** `.agents/factory/bin/temp_site.sh [--empty] [--keep] CMD…`. The
  reviewer must never run a bare `uv run rcac-docs-mcp --index` or otherwise touch the developer's
  real site — `$RCAC_DOCS_SITE` or `~/.local/share/rcac-docs-mcp`, which holds a full RCAC-Docs
  checkout and an index that took minutes to build.
...
- **Scope is narrow:** correctness bugs, GOAL R-ID gaps, invariant violations (auto-CRITICAL for
  §1–§11, HIGH for §12), and scope creep. **No style nits, no speculative hardening** — a gap-hunting
  reviewer manufactures gaps. The one exception is prose: a diff hunk that violates `AGENTS.md`
  § *Prose and comments* is in scope as a §12 finding. Padding costs a reader's confidence in
  infrastructure that has to be trusted to be adopted, and in `INSTRUCTIONS.md` or
  `SERVER_INSTRUCTIONS` it is billed as system-prompt tokens on every downstream call, forever. Scope
  it to the hunk, not to the file — unless the prose is the deliverable, on a `docs/{slug}` branch or
  against a whole-file-census criterion, where the file is the graded surface. The rubric carries the
  full rule.
...
- **Mandatory human gate** when any CONFIRMED finding touches a high-blast-radius file
  (`src/rcac_docs_mcp/index/indexer.py`, `index/database.py`, `index/schema.sql`,
  `src/rcac_docs_mcp/tools.py`, `src/rcac_docs_mcp/site.py`, `Dockerfile`, `docker-entrypoint.sh`,
  `.github/workflows/build-and-push.yml`) or a tool-surface, unauthenticated-posture or
  index-integrity invariant (§1, §2, §4).
- **Bounded loop:** at most two or three review↔build cycles; escalate on non-convergence.

## Procedure

### Step 1 — Pre-flight
Confirm a `feature/`, `fix/` or `docs/` branch; resolve `{slug}` from the branch; confirm `base`
(defaults to `main`); read `kind` from `TECH.md`. The commit `{category}` is the category of the
branch's shape commit (`git log --oneline {base}..HEAD`, oldest entry), not `kind` — the two
taxonomies differ. Capture the head SHA (`git rev-parse HEAD`). If `TECH.md` `status` is not
`in_review`/`done`, note it — the build may be incomplete — and ask whether to proceed.

**Fixture check:** the injected state above says whether `tests/fixtures/RCAC-Docs` is initialized.
When it is absent, 31 tests skip and `temp_site.sh` exits 3, so the executed-evidence spine cannot be
run at all. Initialize it (`git submodule update --init tests/fixtures/RCAC-Docs`) before delegating,
or STOP: a review whose drives could not run is not a review.
...
**Debate check:** if the diff touches a high-blast-radius file or an §1/§2/§4 invariant — the
...
- the instruction: work in the runnable repo; follow the refutation protocol; **run** the relevant
  gates — `uv run pytest -q` (reporting the passed **and** skipped counts, since 31 skips still exit
  0), `.agents/factory/bin/lint.sh [--no-net]`, and behavioral drives through
  `.agents/factory/bin/temp_site.sh [--empty] [--keep]`, never the developer's real site; the MCP
  tools are FastMCP `FunctionTool` objects, so a probe calls `doc_search.fn("slurm")`, not
  `doc_search("slurm")`, and a `TypeError` there is the probe's bug and not a finding; and **keep
  `PLAN.md`, `TECH.md`, `research/` and `META.md` out of context entirely** — do not open
...
- when the locked `GOAL.md` names a landed cycle's retained record under `spec/` as the reference an
  R-ID is graded against — `spec/docs-only-refactor/` is the standing example — that one path,
  declared readable, including a `research/` file, since the ban above is on this branch's artifacts
  and the exception is the cited path, not the category. The rest of `spec/` stays out, and the
  contents stay out of the prompt: bounded and disclosed like the R-ID list above;
...
light second-pass sanity check, dropping anything not backed by cited evidence — including a claimed
green `pytest` with no counts behind it.

This skill carries **no `Edit` tool**, deliberately — it must not mutate the code it is grading. Every
"append" below is therefore a Read-then-`Write`-the-whole-file round trip: read the existing content,
concatenate, write it back. That applies to `REVIEW.md`, to `META.md`, and to a `ROADMAP.md` entry.
Read first every time; a `Write` that skips the read silently truncates the cumulative record.

Then:

1. **Cycle 1:** write `spec/{slug}/REVIEW.md` from
   [`templates/REVIEW.md`](../../factory/templates/REVIEW.md) — verification run,
...
- **Clean (no CONFIRMED):**
  ...
  {sha} --touch` → recommend `/proj-publish`. That recommendation is a deploy recommendation: the
  squash merge to `main` fires `build-and-push.yml` and the Geddes poller rolls the pod.
- **CONFIRMED findings:**
  ...
  `/proj-build` to fix the named R-IDs and invariants. If any CONFIRMED finding hit a
  high-blast-radius file or a §1/§2/§4 invariant, **STOP and require explicit human sign-off** before
  any further step. The sign-off may be given inline; record the clearance in `REVIEW.md` under
  *Human-gate triggers* — which finding fired it, who cleared it, the date, and the grounds — per the
  rubric. Nothing downstream reconstructs that. A CONFIRMED finding meeting the rubric's *Verdict &
  loop* exception — pre-existing behavior a criterion requires preserving — is deferred rather than
  blocked. **Order matters here, and the obvious order is wrong.** `issues/` and `ROADMAP.md` sit
  *outside* `spec/`, so they land inside the pathspec `proj-publish` diffs for post-review drift.
  Write and commit the seed and its `ROADMAP.md` entry **first**, then re-capture
  `sha=$(git rev-parse HEAD)` and pass **that** to `--reviewed-commit` — the `{sha}` read in Step 1 is
  superseded and must not be reused. Approving against the older commit leaves the seed on the wrong
  side of the gate, and `proj-publish` STOPs on a deferral the review itself asked for.
- **PLAUSIBLE only:** surface to the human for triage; do not auto-block.

Every `--verdict` call other than `none` auto-increments the durable `review.cycle` counter in
`TECH.md`. Do not manage it by hand; it is the source of truth for the three-cycle bound and for
REVIEW.md's "Cycle {n}".
...
```markdown
## F<n> — <one-line title>
`origin=proj-review:<step> severity=<high|medium|low> category=<instruction|steering|tooling|template|missing-guidance> status=open target=<best-guess file>`
...
```
Subject at most 72 characters, and **no `Co-Authored-By:` trailer** of any kind. Do not push.
...
### Final report
Verdict, CONFIRMED/PLAUSIBLE counts, human-gate status, the `pytest` passed/skipped counts, R-ID
coverage (including anything taken on trust), and the recommended next step — `/proj-build` to
remediate, or `/proj-publish` when approved. Note the review cycle count; if it is the second or third
cycle without convergence, escalate.

## Examples

- `/proj-review` — blind correctness pass; write `REVIEW.md`; set verdict; route.
- `/proj-review debate` — two independent reviewers for a diff touching the indexer or the FTS schema.
- `/proj-review completeness` — correctness pass plus the separate did-we-ship-everything sub-pass.
- `/proj-review status` — show the current verdict and existing findings; no work.
...
- A requirement the sandbox genuinely cannot observe is not a pass — the recurring case is anything
  that only happens in the deployed pod. Record it in REVIEW.md's "taken on trust" list, and check it
  was already declared in `GOAL.md` or `PLAN.md` §5. A criterion silently downgraded to trust during
  review is itself a finding.
- A verification trap the reviewer hits that is not feature-specific belongs in the rubric's
  *Verification traps* section, added by `/proj-harness` — never in `REVIEW.md`, which the next
  cycle's reviewer is correctly forbidden to open.
