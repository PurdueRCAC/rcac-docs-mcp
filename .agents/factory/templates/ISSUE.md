...
> session does not have to re-derive it. It is **not** graded by `proj-review` and must never be
> copied into `spec/{slug}/GOAL.md` verbatim — `/proj-feature` promotes it, and that is where
...
| `status:` | Means | What `/proj-feature` does with it |
|---|---|---|
...
| `adopted:{slug}` | Promoted; `spec/{slug}/` now owns it. | Nothing. The seed and its ROADMAP entry stay while the cycle is in flight, because it may bounce or be abandoned; `/proj-roadmap` deletes both once the branch lands on `main`. |
...
work was accepted; acceptance is still `/proj-feature` creating the branch and the GOAL.

`kind:` is an open set and need not match the commit category. A `docs` deferral is common here —
five files state the same contract, so a behavior change often leaves one of them stale — and maps
to a `fix/{slug}` or `docs/{slug}` branch at promotion.

`lane: public` lives in `issues/`. Security-sensitive deferrals use `lane: security` and live in
`.security/issues/` — see the deferral table in [`AGENTS.md`](../AGENTS.md). This server is
unauthenticated and public, so when in doubt, use the security lane and ask.

## Problem

<What is wrong today, for whom, and why it matters. Include the evidence the finder had at hand:
`src/rcac_docs_mcp/<module>.py:NNN`, the mechanism, the observed behavior, and the exact command that
shows it. This is the expensive part of a deferral and the part a one-line roadmap seed throws away.>
...
- Found by: <slug + phase, e.g. `fts5-normalizer` P3 — or an out-of-cycle review>
