...
> loop. Written by the lifecycle skills (`proj-feature` / `proj-plan` / `proj-build` / `proj-review`) when
> the **skillset itself** costs something; read by `proj-publish` (surfaced in the PR) and applied by
> `/proj-harness`. This file is **orthogonal** to the `GOAL → PLAN → TECH → REVIEW` spine — it is about
...
> belongs in `GOAL.md` or `REVIEW.md` is **not** a finding. The blind `proj-review` correctness reviewer
...
`/proj-harness` knows what **not** to change. One line each, naming the skill and step. Skip the
...
only `/proj-harness` flips it. `target` is a best-guess file with **no line number** — re-derive the
exact edit at apply time to avoid staleness — written repo-relative and always spelled `.agents/…`,
never through the `.claude` symlink, so `/proj-harness --all` sees one file as one file. If an
...
`| applied | rejected | deferred` (written by `/proj-harness`).
