...
The ``proj-build`` skill runs this at the top of every invocation instead of
...
def _repo_root() -> Path:
    """Locate the repository root, so --all does not depend on the caller's cwd.

    ``git rev-parse`` first, because a git worktree's root is not this file's
    great-grandparent. Falling back to the path walk keeps the script usable in
    an exported tree with no .git directory.
    """
    try:
        out = subprocess.run(
...
def main(argv: list[str]) -> int:
    if argv == ["--all"]:
        spec = _repo_root() / "spec"
        if not spec.is_dir():
            # An empty backlog prints []; a missing directory is a broken checkout
            # and must not be reported as the same thing.
            print(f"no spec/ directory at {spec}", file=sys.stderr)
            return 2
        rows = [_summary(p) for p in sorted(spec.glob("*/TECH.md"))]
