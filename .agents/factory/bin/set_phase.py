...
The ``proj-build`` and ``proj-review`` skills call this to advance state rather
than hand-editing YAML — surgical in-place YAML edits corrupt indentation. It
parses the frontmatter, applies the requested mutations, re-serializes
canonically, and writes the file back atomically with the body preserved
verbatim.
...
Usage examples:
    ...
    uv run .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --add-phase P7 --name "F1 remediation: honor --site in the tool db path" \
    ...
    uv run .agents/factory/bin/set_phase.py spec/<slug>/TECH.md \
        --phase P3 --verify "uv run pytest -q -k normalize" --touch
...
def main(argv: list[str]) -> int:
    ...
    if args.phase:
        ...
        # gate. name/verify may not be blanked; an empty --satisfies/--depends-on clears
        # the list.
    ...
    if args.verdict or args.reviewed_commit is not None or args.blocked_reason is not None:
        ...
        if args.verdict:
            ...
            # A verdict marks one completed review pass; the counter backs the bounded
            # loop. `none` is a reset rather than a pass, so it does not count.
    ...
    write_document(path, dump_document(data, body))
