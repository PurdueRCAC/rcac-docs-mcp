<!--
Retained record of the stage-tracked docs-only refactor, completed 2026-06-26
and superseded by the .agents/ software factory on 2026-08-27.

This predates the factory, so it is a stage list rather than a
GOAL/PLAN/TECH triple. It is kept because its frontmatter `decisions:` map is
the only record of why sse, the auth modes, and a separate index-path override
are absent; those decisions now also live in AGENTS.md and
.agents/factory/invariants.md, which are the current authority. The `plan_id`
below points at a Warp plan stored outside the repository and is not
recoverable from here.

Nothing reads this file. Do not resume it.
-->
